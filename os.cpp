...
string process_str[] =
      { "CALCULATOR", "TICTAC", "CLOCK", "NOTEPAD", "TODO",  "CREATE", "DELETE", "RENAME", "MUSIC","CALENDER","RANDOMNO","UNITCONVERT","FROGGER","TEXTGAME","Sorting"};
 string temp;
const int maxprocess = 15;
sem_t semaphore;
...
void MainMenu()
{
  ...
  system("tput sgr0");
   ...
   displayMessageCentered("| 6-Create A File | 7-Delete A File | 8-Rename A File | 9-Play Music | ",165);
    cout<<endl;
      displayMessageCentered("| 10-Calender | 11-Random No Generator | 12-Unit Converter | 13-Frogger | 14-Text Adventure Game |",165);
    cout<<endl;
    displayMessageCentered("| 15-Sort Number|",165);
    cout<<endl;
...
void processscheduler(){
 int choice=0 ;    //change if process increased
  ...
  exit(0);
   ...
   {
    cout<<"CHECK CORE"<<checkfreecores();
   ...
   system("zenity --warning --width=500 --height=200 --text 'WARNING ! Not Enough RAM Available Process has been added to Waiting Queue'");
...
void *dispatcher(void*argv){
  ...
  if (!process){
    ...
    else if(currentexec==13){ // frogger
  process = fork();
  if (!process){
    system("g++ frogger.cpp -o frogger"); //compile
      system("g++ exec.cpp -o exec");
    temp=to_string(RAM);
    string temp2=to_string(needOfProcesses[currentexec-1]);
    execl("./exec", "./exec", "./frogger", temp.c_str(), temp2.c_str(), NULL);
    
    }

  }
    else if(currentexec==14){ // textadventuregame
  process = fork();
  if (!process){
    system("g++ textgame.cpp -o textgame"); //compile
      system("g++ exec.cpp -o exec");
    temp=to_string(RAM);
    string temp2=to_string(needOfProcesses[currentexec-1]);
    execl("./exec", "./exec", "./textgame", temp.c_str(), temp2.c_str(), NULL);
    
    }

  }
    else if(currentexec==15){ // sorting
  process = fork();
  if (!process){
    system("g++ sorting.cpp -o sorting"); //compile
      system("g++ exec.cpp -o exec");
    temp=to_string(RAM);
    string temp2=to_string(needOfProcesses[currentexec-1]);
    execl("./exec", "./exec", "./sorting", temp.c_str(), temp2.c_str(), NULL);
...
int main(int argc, char* argv[]){
 //system("rm /tmp/myfifo"); 
  ...
  system("clear");
  system(" chmod a+x displaybar.sh");
  system("./displaybar.sh");
  system(" chmod a+x ./oslogo.sh");
  system("./oslogo.sh");
  ...
  needOfProcesses[12] = 900; //frogger
  needOfProcesses[13] = 650; //textadventuregame
  needOfProcesses[14] = 350; //sorting
